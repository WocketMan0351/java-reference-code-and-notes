...
public class UniversityG {
	private String name;
	private List<String> professors;

	public static void main(String[] args) {

		// Create out university, with professors
		UniversityG ASU = new UniversityG("ASU West - Generic");
		...
		try {
			ASU.getProfessors().get(1000);
		} catch (ArrayIndexOutOfBoundsException e) {
			System.out.println(e.getMessage());
		}

		try {
			ASU.getProfessors().find("Turing");
		} catch (ArrayIndexOutOfBoundsException e) {
			System.out.println(e.getMessage());
		}

		try {
			ASU.getProfessors().insertAfter(1000, "Turing");
		} catch (ArrayIndexOutOfBoundsException e) {
			System.out.println(e.getMessage());
		}

		try {
			ASU.getProfessors().insertAfter("Kay", "Turing");
		} catch (ArrayIndexOutOfBoundsException e) {
			System.out.println(e.getMessage());
		}

		try {
			ASU.getProfessors().delete(1000);
		} catch (ArrayIndexOutOfBoundsException e) {
			System.out.println(e.getMessage());
		}
	}

	public UniversityG(String n) {
		name = n;
